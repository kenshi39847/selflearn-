...
理解度チェック
1.Javaの特徴
Javaはオブジェクト指向言語の1つで、オブジェクト指向とは、プログラムの中で扱う対象をモノ（オブジェクト）になぞらえ、
組み合わせによってアプリを形成していく手法のことである。
Javaで作成されたアプリは、Java仮想マシンの上で動作する。仮想マシンがプラットフォームごとの違いを吸収するので、
基本的にJavaアプリは仮想マシンが対応するすべてのプラットフォームで動作する。
また、ガベージコレクションという仕組みによって、メモリ管理が自動化され、アプリ開発者がメモリの開放などを意識する必要はない。
*/
