...
○型パラメーターの制約
<型パラメーター extends 境界型, ...>
境界型には、クラス/インターフェイスいずれも指定できる。ただし、いずれの場合もキーワードはextendsである。
また、境界型として複数の型を指定する場合には「&」で連結する。ただし、この場合、以下の制限がある。
・インターフェイスは複数指定可能、クラスは1つのみ（0でも可）
・クラス→インターフェイスの順で記述する
*/

/*
ジェネリック型としてよく似た概念として、引数/戻り値、ローカル変数などの型を呼び出し時に決められる
メソッド/コンストラクターを「ジェネリックメソッド/ジェネリックコンストラクラー」という。
○ジェネリックメソッド
[修飾子] <型パラメーター, ...> 戻り値型 メソッド名（引数の型 引数名, ...） [throws句] {
  ...メソッドの本体...
}
*/
