...
/*
例外は、その場で処理するばかりではなく、その場ではログを出力するにとどめ、処理そのものは呼び出し元に委ねる
こともあり、これを「例外の再スロー」という。
その時点で適切な回復手段がなければむしろ積極的に上位のメソッドに処理を委ねるべきである。
また、将来的にコードに変更が加わったとき、処理すべき例外も変化し、呼び出し側に影響が及ぶ可能性があり、
このような問題を避ける手法として、「翻訳例外」がある。
翻訳例外とは個々の例外をいったん補足して、上位の例外として束ねたうえで再スローすることである。
翻訳例外を用いることで、呼び出し側でも個々の例外を意識することなく、より高いレベルでの例外だけを意識すれば済む。
翻訳された上位例外から本来の原因（下位例外）を取得するには、getCauseメソッドを利用する。
*/

/*
例外クラスを定義する場合の基本的なルールは以下である。
1.Exception（またはその派生クラス）を継承していること
2.コンストラクターをオーバーライドする
*/
