...
/*
コンストラクターとは別にインスタンスを生成するためのクラスメソッドがあり、
このようなインスタンス生成を目的としたメソッドを「ファクトリーメソッド」という。
ファクトリーメソッドからインスタンスを生成し、戻り値として返すだけである。
ファクトリーメソッドを利用することは以下のような利点がある。
1.自由に命名できる
例）
empty,valueOf,of,from,getInstance,newInstance
2.インスタンスを常に生成しなくてもよい
あらかじめキャッシュしておいたインスタンスを再利用することで、オブジェクト生成のオーバーヘッドを軽減できる。
3.戻り値の型を抽象型/インターフェース型にもできる
この性質を利用することで、実装クラスへの依存をファクトリーメソッドだけにとどめることができる。
*/
